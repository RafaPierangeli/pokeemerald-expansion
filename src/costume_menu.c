...
static void SpriteCallback_UpdateSpritePosition(struct Sprite *sprite)
{
    ...
    if (sprite->x > (xPos_initial + sprite->slotId*slotSize))        //DPAD_RIGHT
    {
        sprite->x -= 4;
    }
    if (sprite->x < (xPos_initial + sprite->slotId*slotSize))        //DPAD_LEFT
    {
        sprite->x += 4;
    }
    if (sprite->x <= xPos_initial - (4*slotSize) || sprite->x >= xPos_initial + (4*slotSize))
    {
        FieldEffectFreeGraphicsResources(sprite);
    }
    else if (sprite->x == xPos_initial + sprite->slotId*slotSize)
    {
...
static u8 GetSpriteIdByXY(s16 x, s16 y)
{
    ...
    for (i = 0; i < MAX_SPRITES; i++)
        if ((gSprites[i].inUse) && (gSprites[i].x == x) && (gSprites[i].y == y))
            return i;
}
